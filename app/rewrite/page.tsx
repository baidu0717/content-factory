...
7. 不论原文是否有价格，改写后必须包含一段费用说明，从素材库「费用说明」分类中选取对应受众的句式，像朋友聊天一样一两句说清楚，X和Y是字面占位字母不填数字，不能单独一行只丢出占位符
8. 景点门票自己预订/排队 → 改为「定制师提前订好，到门口直接进」
9. 禁止使用破折号（——），用句号或逗号代替
10. 用词要口语化：选日常生活中真正会说出来的词，不用书面单字词（从容→轻松）、不用商务正式词（引荐→推荐、打理→帮忙搞定）、不用抽象归纳词（两代人→我们一家六口、整体费用→机票加团费大概X万）

**行程业务背景（必须理解，不能违背）：**
- 住宿：小团默认住酒店，含早餐，不含午晚餐；原文出现「民宿」「房东」等描述时，改写为酒店场景，删除房东相关内容
- 餐饮：午餐晚餐自费，由司导带客人去餐厅，司导可推荐餐厅并帮忙点餐；定制师也会提前推荐餐厅；不存在团餐
- 司导构成：一位司导 = 司机兼导游，一人承担全部职责；法意瑞行程通常是意大利+瑞士一位司导开商务车全程，到日内瓦/苏黎世后客人坐TGV去巴黎，巴黎再换一位新司导；不存在每个城市各配一名司导
- 罗马地陪：罗马市区可能有一位地陪导游协助，但罗马段不用商务车（步行城市）；地陪是由定制师图图提前安排的，不是司导找的；原文若提到罗马有地陪可保留，归属给图图
- 两位司导各自负责不同路段，不同时在场，不存在「两人轮流开同一辆车」的情况

**人名使用规则：**
- 司导数量：严格按原文来，原文提到几位司导就写几位，不合并也不拆分
- 司导命名：可以为司导设定一个「X导」格式的名字（如赵导、李导），也可以全程只用「司导」；若取了名字，全文出现2-3次即可，其余提及用「司导」代替；不要每句都带名字
- 定制师称呼：固定使用「图图」，第一次出现写「定制师图图」；之后具体动作用「图图」；泛指角色或结尾呼吁（如「找到靠谱的定制师」）用「定制师」
- 禁止让司导出现在其负责路段以外的地区（如巴黎司导不能出现在瑞士场景）

**禁止的写法（会产生AI味或广告味）：**
- 禁止文学化感官叠加：「烘焙香」「光影交错」「时光流淌」「空气里弥漫着」类描写
- 禁止排比对仗收尾：「是X得到Y得到的」「不是X，是Y」「是能X的Y」类句式
- 禁止升华式感悟句：「这或许就是XX的意义」「这大概就是XX吧」「突然懂了XX」类作文结尾
- 禁止烂大街情绪套话：「笑得合不拢嘴」「幸福感满满」「治愈了」「美哭了」「泪目」类空洞表达
- 写法基准：像在小红书编辑区直接打字发出去，不是提前写好再润色的文章；允许句子带口语随意感，思路可以跳跃，有真实感比文字精致更重要
                      ...
                      <button
                        ...
                        className={`flex-1 px-3 py-2 text-sm rounded-lg border transition-colors ${
                          selectedModel === 'deepseek-v4-flash'
                            ? 'bg-cyan-100 border-cyan-400 text-cyan-800 font-medium'
