...
- 表达感受时禁止使用散文/书面单字词（如"从容"、"回味"、"恣意"、"治愈"），也不要用中性现象描述替代感受（如"不赶时间的感觉"）；直接用带褒义情绪的口语形容词（如"自由自在"、"轻松"、"舒服"、"踏实"）
- 禁止用官方/书面词替代日常口语：「引荐」→「推荐」，「打理」→「帮忙搞定」，「将就」等语义模糊时直接删掉换具体说法
- 人物称呼要口语化、具体：直接说「我们一家六口」「两个孩子」「爸妈」，禁止用「两代X口之家」「两代人」等泛化归纳表达
- 费用描述要像朋友聊天，简单直白：团费人均X万，加上机票签证大概Y万左右，就结束；禁止写成包含项目清单、禁止用「区间」「浮动」「整体费用」等商业词汇
