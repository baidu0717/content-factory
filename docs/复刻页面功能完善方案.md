# 复刻页面功能完善方案

**创建日期**：2026-02-06
**目的**：完善 `/rewrite` 页面的图片上传、预览和发布功能

---

## 📊 当前功能状态

### ✅ 已完成的功能

#### 1. 图片上传功能
**位置**：`app/rewrite/page.tsx:707-758`

**功能**：
- ✅ 点击或拖拽上传（支持多选）
- ✅ 最多18张图片
- ✅ 支持 JPG、PNG 格式
- ✅ 图片预览（3列网格）
- ✅ 删除图片
- ✅ 显示图片序号

**代码**：
```typescript
const handleFileUpload = (files: FileList | null) => {
  // 验证文件类型
  // 限制最多18张
  // 生成预览URL
}
```

#### 2. 预览功能
**位置**：`components/XiaohongshuPreview.tsx`

**功能**：
- ✅ 图片轮播（3:4比例）
- ✅ 显示标题
- ✅ 显示正文
- ✅ 显示话题标签
- ✅ 图片指示器
- ✅ 左右切换按钮

**调用方式**：
```typescript
<XiaohongshuPreview
  isOpen={showPreview}
  onClose={() => setShowPreview(false)}
  title={editableTitle}
  content={editableContent}
  tags={editableTags}
  images={previewUrls}  // 本地上传的图片预览URL
/>
```

#### 3. AI改写功能
- ✅ 改写标题
- ✅ 改写正文
- ✅ 一键改写全部
- ✅ 自定义提示词
- ✅ 历史版本记录

---

### ⚠️ 部分完成的功能

#### 发布功能
**位置**：`app/rewrite/page.tsx:274-337`

**当前状态**：
```typescript
// TODO: 需要先创建文章记录，然后调用发布API
console.log('[发布] 准备发布文章:', article)
alert('发布功能开发中...')
```

**缺少的步骤**：
1. ❌ 将文章保存到数据库
2. ✅ 上传图片到 Vercel Blob（代码已写，未集成）
3. ✅ 调用小红书发布 API（代码已写，未集成）
4. ❌ 显示二维码供手机扫码

---

### ❌ 未实现的功能

#### 本地图片识别（用户新需求）

**可能的含义**：
1. **方案A**：OCR识别图片中的文字
2. **方案B**：识别图片内容生成描述
3. **方案C**：从图片提取元数据（EXIF）
4. **方案D**：图片分类/标签识别

**需要澄清**：用户希望"识别"图片的什么信息？

---

## 🎯 完善方案

### 方案1：完善发布功能（推荐）

**目标**：让复刻页面的"发布笔记"按钮真正工作

#### 步骤1：创建保存文章的API（如果没有）

**文件**：`app/api/articles/route.ts`

```typescript
export async function POST(req: NextRequest) {
  const { title, content, tags, images } = await req.json()

  const db = getDb()
  const stmt = db.prepare(`
    INSERT INTO articles (title, content, tags, images, status, created_at)
    VALUES (?, ?, ?, ?, 'draft', CURRENT_TIMESTAMP)
  `)

  const result = stmt.run(
    title,
    JSON.stringify(content),
    JSON.stringify(tags),
    JSON.stringify(images)
  )

  return NextResponse.json({
    success: true,
    data: { articleId: result.lastInsertRowid }
  })
}
```

#### 步骤2：修改复刻页面的发布逻辑

**文件**：`app/rewrite/page.tsx:274-337`

```typescript
const handlePublish = async () => {
  // 1. 上传图片到 Vercel Blob
  const formData = new FormData()
  uploadedFiles.forEach(file => formData.append('images', file))

  const uploadResponse = await fetch('/api/upload/images', {
    method: 'POST',
    body: formData
  })
  const { urls: imageUrls } = await uploadResponse.json()

  // 2. 保存文章到数据库
  const saveResponse = await fetch('/api/articles', {
    method: 'POST',
    body: JSON.stringify({
      title: editableTitle,
      content: editableContent,
      tags: editableTags.split(/\s+/),
      images: imageUrls
    })
  })
  const { articleId } = await saveResponse.json()

  // 3. 调用发布API
  const publishResponse = await fetch('/api/xiaohongshu/publish', {
    method: 'POST',
    body: JSON.stringify({ articleId })
  })
  const result = await publishResponse.json()

  // 4. 显示二维码
  setPublishResult({
    qrCodeUrl: result.data.qrCodeUrl,
    noteId: result.data.noteId
  })
}
```

#### 步骤3：添加二维码显示界面

**文件**：`app/rewrite/page.tsx`

```tsx
{publishResult && (
  <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
    <div className="bg-white rounded-2xl p-6 max-w-sm">
      <h3 className="text-lg font-bold mb-4">扫码发布到小红书</h3>
      <img src={publishResult.qrCodeUrl} alt="发布二维码" className="w-full" />
      <p className="text-sm text-gray-500 mt-4 text-center">
        请用手机微信扫码完成发布
      </p>
      <button
        onClick={() => setPublishResult(null)}
        className="w-full mt-4 px-4 py-2 bg-gray-200 rounded-lg"
      >
        关闭
      </button>
    </div>
  </div>
)}
```

---

### 方案2：本地图片识别功能

#### 如果是"OCR识别图片文字"

**技术方案**：
1. 使用 Gemini Vision API（已有 API Key）
2. 或使用 OCR.space 等免费API

**实现步骤**：
```typescript
// 1. 上传图片后自动识别
const handleFileUpload = async (files: FileList | null) => {
  // 上传图片
  setUploadedFiles(newFiles)

  // 自动OCR识别
  for (const file of newFiles) {
    const text = await recognizeText(file)
    console.log('识别到的文字:', text)
  }
}

// 2. OCR识别函数
async function recognizeText(file: File): Promise<string> {
  const formData = new FormData()
  formData.append('image', file)

  const response = await fetch('/api/ocr', {
    method: 'POST',
    body: formData
  })

  const result = await response.json()
  return result.text
}
```

#### 如果是"识别图片内容生成描述"

**使用 Gemini Vision API**：

```typescript
async function describeImage(file: File): Promise<string> {
  const base64 = await fileToBase64(file)

  const response = await fetch('/api/gemini/vision', {
    method: 'POST',
    body: JSON.stringify({
      image: base64,
      prompt: '请描述这张图片的内容，适合用作小红书笔记配图说明'
    })
  })

  const result = await response.json()
  return result.description
}
```

#### 如果是"提取图片元数据"

**使用 exif-js 库**：

```typescript
import EXIF from 'exif-js'

function extractMetadata(file: File) {
  EXIF.getData(file, function() {
    const make = EXIF.getTag(this, 'Make')        // 相机品牌
    const model = EXIF.getTag(this, 'Model')      // 相机型号
    const date = EXIF.getTag(this, 'DateTime')    // 拍摄时间
    const gps = EXIF.getTag(this, 'GPSLatitude')  // GPS位置

    console.log('相机:', make, model)
    console.log('时间:', date)
  })
}
```

---

## 🔑 第三方API配置

### MyAIBot API（小红书发布）

**文档**：https://www.myaibot.vip/docs/api

**配置文件**：`.env.local`
```env
MYAIBOT_API_KEY=rn_95b1c43fa86396bbb84b2a1c86863d7303c6d40cc217e69ea7133296cff2
```

**API端点**：
```
POST https://www.myaibot.vip/api/rednote/publish
```

**请求格式**：
```json
{
  "api_key": "rn_95b1c43...",
  "type": "normal",
  "title": "笔记标题（≤20字）",
  "content": "正文内容（≤1000字）",
  "images": [
    "https://image-url-1.jpg",
    "https://image-url-2.jpg"
  ]
}
```

**响应格式**：
```json
{
  "success": true,
  "data": {
    "id": "note_123",
    "note_id": "64f8e7a1000000001a00b2c3",
    "title": "笔记标题",
    "qrCodeUrl": "https://www.myaibot.vip/qr/xxx.png",
    "publishUrl": "https://www.xiaohongshu.com/discovery/item/xxx",
    "coverImage": "https://image-url-1.jpg",
    "createdAt": 1627891234567
  },
  "message": "发布成功！请扫描二维码在手机端完成发布"
}
```

**图片要求**：
- 格式：PNG、JPG、JPEG、WEBP（不支持GIF）
- 数量：最多18张
- 大小：单张 ≤ 32MB
- 分辨率：≥ 720×960像素
- **重要**：图片必须是可公开访问的URL（需要先上传到Vercel Blob）

---

### Vercel Blob（图片存储）

**已配置**：
```typescript
import { put } from '@vercel/blob'

// 上传单张图片
const blob = await put(filename, file, {
  access: 'public'
})
console.log(blob.url) // 返回可访问的URL
```

**API端点**：`/api/upload/images`（需检查是否存在）

---

### Gemini Vision API（图片识别，可选）

**配置文件**：`.env.local`
```env
GEMINI_TEXT_API_KEY=AIzaSy***已隐藏***（从环境变量获取）
```

**用途**：
- OCR识别图片文字
- 生成图片描述
- 图片内容分析

---

## 📝 开发优先级

### 高优先级（必须完成）

1. **完善发布功能**
   - [ ] 检查/创建 `/api/upload/images` 端点
   - [ ] 检查/创建 `/api/articles` 端点
   - [ ] 修改 `app/rewrite/page.tsx` 的 `handlePublish` 函数
   - [ ] 添加二维码显示界面
   - [ ] 测试完整发布流程

### 中优先级（建议完成）

2. **优化图片上传体验**
   - [ ] 添加上传进度条
   - [ ] 支持拖拽排序图片
   - [ ] 图片压缩优化（大于5MB自动压缩）
   - [ ] 批量删除图片

3. **完善预览功能**
   - [ ] 添加分享按钮
   - [ ] 支持全屏预览
   - [ ] 显示图片尺寸信息

### 低优先级（可选）

4. **图片识别功能**（需要先澄清需求）
   - [ ] OCR识别图片文字
   - [ ] 或 AI生成图片描述
   - [ ] 或 提取图片元数据

---

## ❓ 需要澄清的问题

### 关于"本地图片识别"

**请明确以下问题**：

1. **识别什么信息？**
   - A. 识别图片中的文字（OCR）
   - B. 识别图片内容并生成描述
   - C. 识别图片拍摄信息（相机、时间、地点）
   - D. 识别图片主题/标签

2. **识别后做什么？**
   - A. 自动填充到正文中
   - B. 作为标签建议
   - C. 用于图片排序
   - D. 仅供参考查看

3. **何时触发识别？**
   - A. 上传图片时自动识别
   - B. 点击按钮手动识别
   - C. 在预览时显示识别结果

---

## 🚀 快速开始（完善发布功能）

### 1. 检查现有API端点

```bash
# 检查是否存在图片上传API
ls app/api/upload/images/route.ts

# 检查是否存在文章保存API
ls app/api/articles/route.ts
```

### 2. 如果缺少，我可以帮你创建

告诉我：
- [ ] 需要创建图片上传API吗？
- [ ] 需要创建文章保存API吗？
- [ ] 需要修改发布逻辑吗？
- [ ] 需要添加二维码显示界面吗？

### 3. 测试发布流程

1. 上传1-18张图片
2. 编辑标题、正文、标签
3. 点击"预览笔记"确认效果
4. 点击"发布笔记"
5. 等待上传和处理
6. 看到二维码
7. 手机扫码完成发布

---

## 📚 相关文档

- [MyAIBot API文档](https://www.myaibot.vip/docs/api)
- [小红书图片规范](./图片上传和发布指南.md)
- [Vercel Blob文档](https://vercel.com/docs/storage/vercel-blob)
- [Gemini Vision API](https://ai.google.dev/gemini-api/docs/vision)

---

## 🎯 下一步行动

**请告诉我**：

1. 是否需要我现在完善发布功能？（高优先级）
2. "本地图片识别"的具体需求是什么？（需要澄清）
3. 是否有其他功能需求？

我可以立即开始编写代码！

---

**文档维护者**：Claude Code
**最后更新**：2026-02-06
**相关文件**：
- `app/rewrite/page.tsx` - 复刻页面
- `components/XiaohongshuPreview.tsx` - 预览组件
- `app/api/xiaohongshu/publish/route.ts` - 发布API
