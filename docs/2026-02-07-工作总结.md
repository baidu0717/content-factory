...
#### 背景
"生成复刻链接"功能用于批量为飞书表格生成复刻URL，但这是一个后台管理功能，不应该在前端导航菜单中展示。

#### 改动内容

**删除的文件**：
- `app/generate-links/page.tsx` (212行代码)

**修改的文件**：
- `components/Sidebar.tsx` - 从导航菜单移除该选项

**保留的功能**：
- ✅ 保留 `/api/feishu/generate-rewrite-links` API
- ✅ API 用于为飞书表格批量生成复刻链接

**功能说明**：
```typescript
// API: POST /api/feishu/generate-rewrite-links
// 功能：
// 1. 读取飞书表格中所有笔记记录
// 2. 为每条记录生成带参数的URL：
//    /rewrite?title=xxx&content=xxx&tags=xxx
// 3. 将URL写入"去复刻"或"复刻链接"字段
```

**使用流程**：
1. 用户在飞书表格中点击"去复刻"按钮
2. 自动跳转到 `/rewrite` 页面
3. 标题、正文、标签通过URL参数自动填充
4. 用户可以直接点击"一键改写全部"开始复刻

**导航菜单精简为 5 个核心功能**：
```typescript
const menuItems = [
  { href: '/rewrite', label: '小红书复刻' },
  { href: '/xiaohongshu-extract', label: '小红书采集' },
  { href: '/collect', label: '批量采集' },
...
```
删除: 212 行代码
修改: 1 个文件 (Sidebar.tsx)
```

**提交记录**：
- `refactor: 移除"生成复刻链接"前端页面 - 改为后台功能`
...
3. `components/Sidebar.tsx` - 精简导航菜单，删除今日统计，移除生成链接菜单
4. `app/page.tsx` - 改为重定向页面

### 删除的文件
1. `app/analysis/page.tsx` - 选题分析页面
2. `app/create/page.tsx` - 内容创作页面
3. `app/monitoring/page.tsx` - 定时监控页面
4. `app/settings/page.tsx` - 设置页面
5. `app/generate-links/page.tsx` - 生成复刻链接页面
...
### 详细统计
- **小红书预览界面优化**: 修改 1 个文件，重构代码
- **图片拖拽排序**: 修改 1 个文件，+53 行
- **系统功能精简**: 删除 4 个页面，修改 2 个文件，-4109 行
- **移除生成链接页面**: 删除 1 个页面，修改 1 个文件，-212 行
...
**后台功能** (无前端页面):
- `/api/feishu/generate-rewrite-links` - 为飞书表格生成复刻链接
...
1. ✅ 从飞书表格"去复刻"按钮跳转
2. ✅ 标题、正文、标签自动填充
3. ✅ API `/api/feishu/generate-rewrite-links` 正常工作
...
**更新记录**:
- 2026-02-07 15:00: 初始版本（小红书预览优化 + 拖拽排序 + 功能精简）
- 2026-02-07 17:30: 更新版本（移除"生成复刻链接"前端页面）
